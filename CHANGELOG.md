...
### Added
- **Local Home Assistant intent handling (opt-in)** — new **Local HA intent handling** option (`off` / `answers` / `commands`, default `off`). In `answers` mode Hermes uses HA's side-effect-free recogniser and only processes known read-only query intents locally, returning results such as room temperature or entity state in milliseconds. `commands` mode additionally executes house commands that HA matches on the intact transcript. Both modes are off by default: requests handled locally bypass the Hermes Home Assistant plugin's allow/block lists and audit log (see README → *Local HA intent handling*). ([#46](https://github.com/rusty4444/hermes-voice-ha-integration/pull/46), thanks @byjaps.)
- Behavioural test coverage for the conversation agent: `QUERY_ANSWER` with and without speech, side-effect-aware command handling, error/unmatched fallback, Hermes-payload preservation, non-speech-marker cleanup, legitimate trailing text, multi-clause preservation, options-flow persistence, the opt-in contract, and the shared query timeout.

### Fixed
- Transcript cleanup no longer changes what is sent to Hermes. Only whisper.cpp non-speech annotations (`[música]`, `(risos)`, `[BLANK_AUDIO]` …) are dropped, and only from the copy used for the local attempt; legitimate trailing text such as `send Sam a notification (urgent)` or `como se diz obrigado?` is untouched.
...
- `answers` mode now recognises a read-only query intent before calling HA's native agent, preventing commands from executing locally and then falling through to Hermes for a second execution.

...
- Plugin unload, disable and forced reload now close active Home Assistant WebSockets before cleaning up the listener, terminate the receiver thread, and release its captured assist handler. This prevents a client remaining attached to an orphaned stale receiver after the replacement has bound.

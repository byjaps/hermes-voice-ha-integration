...
### Fixed
- The bundled Hermes `voice_stack` plugin no longer warns `address already in use` on port 7860 when another Hermes process already serves it. The receiver is a process-wide singleton, but every Hermes process that loads the plugin (gateway, CLI session, dashboard) re-imports the module with fresh globals and re-attempted the bind, logging a failure for a port the first process was serving correctly. `start_ws_receiver` now probes the port first and, when the receiver's own unauthenticated `/health` payload identifies it (`service == "hermes-ha-ws"`), logs an informational "already served by another Hermes receiver" instead — while a foreign service holding that port, or a Hermes receiver serving a different path, still warns. Identity is taken from the health payload rather than the HTTP status, since a protected foreign API answers 401 and an unrelated upgrade endpoint answers 426.
- The in-process receiver record is now owner-scoped: it carries the module file, its code fingerprint, the bind config and the pid. Another profile's plugin no longer adopts a receiver it did not start (nor can `stop_ws_receiver()` stop it), and a module reloaded with changed code rebinds instead of being served by an object still bound to the previous module's globals and assist handler. Wildcard binds are probed on both loopback addresses so an IPv6-only receiver is recognised.

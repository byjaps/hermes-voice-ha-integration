...
- The receiver thread now preserves the owning profile's ContextVar scope, so Assist callbacks resolve that profile's model configuration and credentials instead of falling back to the process launch profile. Plugin unload, disable and forced reload also stop the listener and release its captured assist handler.
